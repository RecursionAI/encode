"""Persistent shell sessions via a long-lived bash process.

A :class:`Terminal` wraps one bash subprocess. State (cwd, env vars, sourced
venvs) persists across ``run()`` calls because every command executes in the
same shell. Use :class:`CommandResult` to inspect output, exit code, and the
post-command cwd.
...
Terminals are *stateful tools* in the Managed Agents sense: they expose
``run(command)`` and own a sandbox-like execution environment. Wrap one in
a callable to use it as a tool through the ToolExecutor seam.
"""
...
from .errors import TerminalError, TerminalTimeoutError
...
def _check_platform() -> None:
    if sys.platform.startswith("win"):
        raise TerminalError(
            "encode.Terminal is only supported on macOS and Linux"
        )
...
class CommandResult(BaseModel):
    """Result of a single ``Terminal.run()`` call.
...
class TerminalSnapshot(BaseModel):
    """Point-in-time view of a Terminal's state.

    Useful for logging, intercept-driven decisions, or persisting alongside
    a Session. Does not include the underlying process handle.
    """
...
class Terminal:
    """Persistent bash terminal backed by ``pexpect.spawn``.
    ...
    def __init__(
        ...
        try:
            ...
            raise TerminalError(f"pexpect is required for terminals: {e}") from e
        ...
        except TerminalTimeoutError as e:
            self.kill()
            raise TerminalError(
                f"bash terminal failed to initialize within {timeout}s"
            ) from e
    ...
    def _spawn(self) -> None:
        try:
            ...
            raise TerminalError(f"failed to spawn bash: {e}") from e
    ...
    def snapshot(self) -> TerminalSnapshot:
        """Return a point-in-time view of the terminal's state."""
    ...
    def run(self, command: str, *, timeout: float | None = None) -> CommandResult:
        if self._proc is None or not self._proc.isalive():
            raise TerminalError(
                "terminal is not running; spawn was killed or never started"
            )
        ...
        try:
            ...
            raise TerminalError(f"terminal read failed: {e}") from e
...
class AsyncTerminal:
    """Persistent bash terminal backed by ``asyncio.create_subprocess_shell``.

    Same surface as :class:`Terminal` but async. Spawns lazily on the first
    ...
    async def _ensure_started(self) -> None:
        ...
        try:
            ...
            await self.kill()
            raise TerminalError(
                f"bash terminal failed to initialize within {self._default_timeout}s"
            ) from e
    ...
    async def run(self, command: str, *, timeout: float | None = None) -> CommandResult:
        ...
        if proc.stdin is None or proc.stdout is None:
            raise TerminalError("terminal pipes are not available")
        t = self._default_timeout if timeout is None else timeout
        async with lock:
            full = (command + self._sentinel + "\n").encode()
            start = time.perf_counter()
    ...
    async def start(self, command: str) -> None:
        ...
        if proc.stdin is None:
            raise TerminalError("terminal stdin is not available")
    ...
    async def read(self, timeout: float = 1.0) -> str:
        ...
        if proc.stdout is None:
            raise TerminalError("terminal stdout is not available")
        buf = self._buffer
        self._buffer = b""
        try:
